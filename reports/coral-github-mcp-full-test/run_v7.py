"""
Fix v7 (FINAL): use correct filter VALUE FORMATS, not just correct column names.

Key insight from debugging: ref='main' fails but ref='heads/main' works.
The ref value needs the full ref prefix, not just the short name.
"""
...
def run_sql(query, timeout=25):
    ...
    try:
        send({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"v7","version":"1.0"}}})
        read_response(1, 20)
        send({"jsonrpc":"2.0","method":"notifications/initialized","params":{}})
        send({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sql","arguments":{"queries":[query],"intent":"v7","task_id":TASK_ID}}})
...
def fetch_one(query, value_col):
    ...
    def read_response(eid, t):
        ...
        while True:
            ...
            if o.get("id") == eid: return o
    try:
        send({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"v7b","version":"1.0"}}})
        read_response(1, 20)
        send({"jsonrpc":"2.0","method":"notifications/initialized","params":{}})
        send({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sql","arguments":{"queries":[query],"intent":"v7b","task_id":TASK_ID}}})
...
# CORRECT VALUE FORMATS based on actual coral/git conventions:
# - ref: needs full prefix like 'heads/main' or 'tags/v0.9.0'
# - branch: short name OK ('main')
# - tag: short name OK ('v0.9.0')
# - commit_sha: full sha
# - job_id, run_id: numeric
# - actor_id, user_id: numeric
# - pull_number, issue_number: numeric (PR/issue number)

# Resolvers now return BOTH the value AND the format hint
# format_function tells the query builder how to format the value
RESOLVERS = {
    "run_id": [
...
# Value formatters — know the correct format for each filter
FORMATTERS = {
    "ref": lambda v: f"heads/{v}" if not v.startswith("heads/") and not v.startswith("tags/") else v,
    "tag": lambda v: v,
    "tag_sha": lambda v: v,
    "branch": lambda v: v,
    "head_sha": lambda v: v,
    "commit_sha": lambda v: v,
    "sha": lambda v: v,
    "tree_sha": lambda v: v,
    "file_sha": lambda v: v,
    "run_id": lambda v: str(v),
    "job_id": lambda v: str(v),
    "pull_number": lambda v: str(v),
    "issue_number": lambda v: str(v),
    "release_id": lambda v: str(v),
    "asset_id": lambda v: str(v),
    "check_run_id": lambda v: str(v),
    "check_suite_id": lambda v: str(v),
    "review_id": lambda v: str(v),
    "thread_id": lambda v: str(v),
    "comment_id": lambda v: str(v),
    "alert_number": lambda v: str(v),
    "gist_id": lambda v: str(v),
    "codespace_name": lambda v: v,
    "deployment_id": lambda v: str(v),
    "pages_deployment_id": lambda v: str(v),
    "hook_id": lambda v: str(v),
    "runner_id": lambda v: str(v),
    "runner_group_id": lambda v: str(v),
    "workflow_id": lambda v: str(v),
    "codeql_variant_analysis_id": lambda v: str(v),
    "classroom_id": lambda v: str(v),
    "assignment_id": lambda v: str(v),
}
...
def format_value(filter_name, raw_value):
    fmt = FORMATTERS.get(filter_name)
    if fmt:
        return fmt(raw_value)
    return raw_value
...
def build_query(name, required_filters):
    ...
    for f in req:
        ...
        if v is not None:
            formatted = format_value(f, v)
            clauses.append(f"{f}={quote(formatted)}")
            resolved[f] = formatted
            continue
        if f in STATIC:
...
# Get fresh required_filters from coral.filters (with dedup)
...
def read2(eid, t):
    ...
    while True:
        ...
        if o.get("id") == eid: return o
send2({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"v7c","version":"1.0"}}})
read2(1, 20)
send2({"jsonrpc":"2.0","method":"notifications/initialized","params":{}})
send2({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sql","arguments":{"queries":[filters_query],"intent":"v7c","task_id":TASK_ID}}})
resp = read2(2, 25)
sc = resp.get("result", {}).get("structuredContent", {})
rows = sc.get("results", [[]])[0].get("rows", []) if sc.get("results") else []
proc.kill()
...
print(f"Loaded filters for {len(req_filters)} tables", flush=True)
...
for name in to_fix:
    ...
    if status in ("data", "empty"):
        ...
        sys.stderr.write(f"[{i}/{len(to_fix)}] {name:50s} {emoji} {status} rows={rows} t={elapsed:.1f}s q={q[:80]}\n")
    elif status == "crash":
    ...
    sys.stderr.flush()
    if i % 5 == 0:
        json.dump(results, open(os.path.join(ROOT, "v7_results.json"), "w"), default=str, indent=2)
